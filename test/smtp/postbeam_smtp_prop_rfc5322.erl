...
-module(postbeam_smtp_prop_rfc5322).
...
prop_encode_no_crash() ->
    ?FORALL(
        ...
        is_binary('Elixir.Postbeam.SMTP.Util':combine_rfc822_addresses(AddressList))
    ).
...
prop_encode_scan_no_crash() ->
    ?FORALL(
        ...
        ?LET(Opts, use_unicode(), gen_address_list(Opts)),
        begin
            Encoded = 'Elixir.Postbeam.SMTP.Util':combine_rfc822_addresses(AddressList),
            Res = postbeam_smtp_rfc5322_scan:string(unicode:characters_to_list(Encoded)),
            ?WHENFAIL(
                io:format(
...
prop_encode_decode_match() ->
    ?FORALL(
        ...
        begin
            Encoded = 'Elixir.Postbeam.SMTP.Util':combine_rfc822_addresses(AddressList),
            Res = 'Elixir.Postbeam.SMTP.Util':parse_rfc5322_addresses(Encoded),
            ?WHENFAIL(
                io:format(
                    ...
                    [
                        ...
                        Encoded,
                        Res,
                        postbeam_smtp_rfc5322_scan:string(unicode:characters_to_list(Encoded))
                    ]
                ),
                begin
                    {ok, Decoded} = Res,
...
prop_encode_decode_group() ->
    ?FORALL(
        ...
        begin
            ...
            {ok, Tokens, _} = postbeam_smtp_rfc5322_scan:string(unicode:characters_to_list(Encoded)),
            Res = postbeam_smtp_rfc5322_parse:parse(Tokens),
            ?WHENFAIL(
                io:format(
                    "Name: '~p'~n"
                    "AddressList: ~p~n"
...
encode_group(Name, AddressList) ->
    EncodedList = 'Elixir.Postbeam.SMTP.Util':combine_rfc822_addresses(AddressList),
    EncName =
