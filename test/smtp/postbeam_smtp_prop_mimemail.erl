...
-module(postbeam_smtp_prop_mimemail).
...
prop_plaintext_encode_no_crash() ->
    ?FORALL(
        Mail,
        gen_plaintext_mail(),
        is_binary('Elixir.Postbeam.SMTP.MIME':encode(Mail))
    ).
...
prop_plaintext_encode_decode_match() ->
    ?FORALL(
        ...
        gen_plaintext_mail(),
        begin
            Encoded = 'Elixir.Postbeam.SMTP.MIME':encode(Mail),
            Recoded = 'Elixir.Postbeam.SMTP.MIME':decode(Encoded),
            ?WHENFAIL(
                io:format(
...
prop_encode_decode_no_mime_version_match() ->
    ?FORALL(
        ...
        begin
            Encoded = 'Elixir.Postbeam.SMTP.MIME':encode(Mail),
            Recoded = 'Elixir.Postbeam.SMTP.MIME':decode(
    ...
    maps:fold(
        fun
            ...
            ->
                ...
                lists:foreach(
                    fun({PKA, PVA}) ->
                        ?assert(lists:member({'Elixir.Postbeam.SMTP.Binary':to_lower(PKA), PVA}, KVB))
                    end,
                    KVA
                );
            (K, V, _) ->
...
prop_quoted_printable() ->
    ?FORALL(
        ...
        begin
            [QPEncoded] = 'Elixir.Postbeam.SMTP.MIME':encode_quoted_printable(Body),
            ?assertEqual(Body, 'Elixir.Postbeam.SMTP.MIME':decode_quoted_printable(QPEncoded)),
...
prop_smtp_compatible() ->
    ?FORALL(
        ...
        begin
            SevenByte = 'Elixir.Postbeam.SMTP.MIME':encode(Mail),
