...
for DOMAIN in "mx1.example.com" "mx2.example.com"; do
	...
	openssl req -new -key $KEY -out $CSR -subj "/CN=${DOMAIN}" -addext "subjectAltName=DNS:${DOMAIN}"
	# create signed server cert
	openssl x509 -req -days 3650 -in $CSR -CA ${DATADIR}/root.crt -CAkey ${DATADIR}/root.key -CAcreateserial -copy_extensions copy -out $CRT
done
