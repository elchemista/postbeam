...
## Checklist for new LiveViews

* [ ] Wrapped in `<Layouts.app flash={@flash} current_scope={@current_scope} ...>`
* [ ] Route placed in the correct `live_session`
* [ ] No `@current_user`; uses `@current_scope.user`
* [ ] Uses streams for collections and `phx-update="stream"`
* [ ] Forms use `to_form/2` + `<.form for={@form}>` + `<.input>`
* [ ] Buttons use `<.button>`, icons use `<.icon>`
* [ ] Uses Req for HTTP
* [ ] All strings wrapped with `gettext`
